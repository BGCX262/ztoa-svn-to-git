 ...
 * 加载时的提示信息
 * 2013年5月2日 20:48:48
 * @param dataid 显示提示信息的id
 * @param cols   表格的colspan数量
 * @param imgurl  loading图片的路径
 * @param loadtext  提示的文字信息
 */
function comload(dataid,cols,imgurl,loadtext){
    $('#'+dataid).html("");
    tr = document.getElementById(dataid).insertRow(-1);//
    ...
    img.src=imgurl+'statics/images/loading.gif';

    td.appendChild(img);
    td.appendChild(document.createTextNode(loadtext));
}
 ...
 * 2013年5月2日 20:48:40
 * @param dataid 显示的id
 * @param cols   表格colspan数目
 * @param notext 提示文字信息
 */
function nodata(dataid,cols,notext){
    ...
    td.colSpan = cols;
    td.className = 'acenter';
    td.appendChild(document.createTextNode(notext));
}

/**
 * 行点击换颜色，覆盖变色，移开恢复原来的颜色
 * @param dataid 增加行的id
 */
function trchangcolor(dataid){
...
/**
 *container_id  隐藏和显示的容器id
 */
function openclose(container_id){

      var acl = $("#"+container_id).attr("class");
    if(acl == 'disnone'){
        $("#"+container_id).attr("class","disinlie");
    }else{
        $("#"+container_id).attr("class","disnone");
    }
}

/**
 * 文本区域id含有LineDesc、dongtai的高度自动适应
 */
function autosize(){
    //$("textarea[id*='dongtai']").TextAreaAutoSize({ Height: 40 });
    //$("textarea[id*='LineDesc']").TextAreaAutoSize({ Height: 60 });
    $("textarea[id*='LineDesc']").textareaAutoHeight({minHeight:60});
}
...
function taboperate(){
    ...
    $("#explore-nav li a input").click(function(){
        ...
        Wind.use('artDialog', function () {
                art.dialog({
                   ...
                   // background: '#666', // 背景色
                    ...
                    ok: function () {
                        var tit_old = $("#explore-nav li");

                        if(tit_old.length >= 2){
                            var rel = $(_self).parent()[0].rel;
                            if(rel){
                                $("#ex-" + rel).remove();
                                $("#" + rel).remove();
                                ...
                                var d1 = SimpleDate.strtoDate(startdate);


                                var tit = $("#explore-nav li");//li num after deleted
                                var titlen = tit.length;
                                var next_titlen = titlen + 1;

                                for(var j = 0; j < titlen; j++){
                                    var t = tit[j];

                                    if(j == 0){
                                        $("#"+t.id + " a")[0].className='current';
                                        //console.log($("#"+t.id+" a"));
                                    }else{
                                        $("#"+t.id + " a")[0].className='';
                                    ...
                                    tida.appendChild(document.createTextNode(atit));

                                    if(startdate){
                                        var titdate = new Date(d1.getFullYear(),d1.getMonth(),d1.getDate()+j).Format("yyyy-MM-dd w");

                                        if(j < 10){
                                            tida.appendChild(document.createTextNode(titdate + '\u00A0'));
                                        }else{
                                            tida.appendChild(document.createTextNode(titdate));
                                        }
                                    }
                                ...
                                for(var i = 0; i < cont.length; i++){
                                    ...
                                    if(i == 0){
                                        $("#"+c.id).css("display",'');
                                        //$("#"+c.id).className = '';
                                        $("#"+c.id).removeClass("disnone");

                                        //console.log($("#"+c.id).className);
                                    }else{
                                        $("#"+c.id).className = 'disnone';

                                    }
                                }
                            }
                        }else{
                            art.dialog.alert('请至少保留一个！');
                        }
...
function addtab(startdate){
   ...
   var startdate = '';
    ...
    var tit = $("#explore-nav li");//li num before add
    var titlen = tit.length;
    var next_titlen = titlen + 1;
    ...
    /*var span = document.createElement("span");
        span.className = 'lazyTabClose1';
        //span.backgroundColor = '#d30000';
        span.style.backgroundColor = 'red';
        span.appendChild(document.createTextNode('X'));*/
    var input = document.createElement("input");
        input.type = "button";
        input.className = "closepng";

    var b = document.createElement("b");
        b.id= "p-" + unique;
        b.appendChild(document.createTextNode("第"+next_titlen+"天"));

        if(startdate){
            var titdate = new Date(d1.getFullYear(),d1.getMonth(),d1.getDate()+titlen).Format("yyyy-MM-dd w");

            if(titlen < 10){
                b.appendChild(document.createTextNode(titdate + '\u00A0'));
            }else{
                b.appendChild(document.createTextNode(titdate));
            }
        }
    ...
    document.getElementById("explore-nav").appendChild(li);


    var ul1 = document.createElement("ul");
        ul1.id = unique;
        ul1.className = 'disnone';
    ...
    var td = tr.insertCell(-1);
        ...
        var input = document.createElement("input");
            input.type = 'text';
            input.className = 'input w80';

    td.appendChild(document.createTextNode("区间"));
    ...
    var input = document.createElement("input");
    input.type = 'text';
    input.className = 'input w70';

    td.appendChild(document.createTextNode("交通工具"));
    td.appendChild(input);
    ...
    td.appendChild(document.createTextNode("班次"));
    td.appendChild(input);
    ...
    td.appendChild(document.createTextNode("住宿"));
    td.appendChild(input);
    ...
    td.appendChild(input);
    td.appendChild(document.createTextNode("早"));
    td.appendChild(input1);
    td.appendChild(document.createTextNode("中"));
    td.appendChild(input2);
    td.appendChild(document.createTextNode("晚"));
    td.appendChild(input3);
    td.appendChild(document.createTextNode("自理"));
    ...
    var textarea = document.createElement("textarea");
        textarea.id = unique + '_LineDesc';
        textarea.className = "xctext";

    td.appendChild(textarea);


    li1.appendChild(table);
    ul1.appendChild(li1);

    document.getElementById("all-list-wrap").appendChild(ul1);
...
//所有加了gys_dialog类名的a链接，自动弹出它的href
if ($('a.gys_dialog').length) {
    Wind.use('artDialog', 'iframeTools', function () {
        $('.gys_dialog').on('click', function (e) {
            e.preventDefault();
            ...
            art.dialog.open($(this).prop('href'), {
                ...
                button: [{
                    ...
                    callback: function () {
                        ...
                        var radioname = iframe.document.getElementsByName("wtfperid");
                        var dataurl = iframe.document.getElementById("dataurl").value;
                        var wtfid = 0;

                        for(var i = 0; i < radioname.length; i++){
                            if(radioname[i].checked == true) {
                                 wtfid = radioname[i].value;
                            }else{
                                continue;
                            }
                        }
                        ...
                        $.getJSON(dataurl+'&wtf_id='+wtfid,function(data){
                           ...
                           /* $("#wtf_id").val('');
                            ...
                            $("#wtf_id").val(data.wtf_id);
                            $("#wtfname").val(data.wtf_name);
                            $("#dept").val(data.dept);
                            $("#wtf_group").val(data.wtf_group);
                            $("#wtf_name").val(data.name);
                            $("#wtf_tell").val(data.tell);
                            $("#wtf_fax").val(data.fax);
                            $("#wtf_phone").val(data.phone);
                        });
