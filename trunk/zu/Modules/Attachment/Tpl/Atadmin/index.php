...
<div class="wrap J_check_wrap">
  ...
  <form method="get" action="">
    ...
    <div class="search_type cc mb10">
      ...
      上传时间：
        <input type="text" name="start_uploadtime" class="input length_2 J_date" value="{$start_uploadtime}" style="width:80px;">
        -
        <input type="text" class="input length_2 J_date" name="end_uploadtime" value="{$end_uploadtime}" style="width:80px;">
        附件类型：
        <input type="text" class="input length_2" name="fileext" style="width:80px;" value="{$fileext}" placeholder="类型：jpg、png">
        使用状态：
        <select name="status" id="status" >
            <option value="" <if condition="$status eq '' "> selected</if>>全部</option>
            <option value="1" <if condition="$status eq '1' "> selected</if>>已经在使用</option>
            <option value="0" <if condition="$status eq '0' "> selected</if>>没有被使用</option>
          </select> 
          附件名称：
        <input type="text" class="input length_2" name="keyword" style="width:200px;" value="{$filename}" placeholder="请输入附件名称...">
  ...
  <table width="100%" cellspacing="0">
      ...
      <volist name="data" id="vo">
        <tr>
          ...
          <td ><img src="{$config_siteurl}statics/images/ext/{$vo.fileext}.gif" />{$vo.filename} 
          ...
          <td align="center">{$vo.uploadtime}</td>
          <td align="center"><a href="javascript:preview({$vo.aid}, '{$vo.filename}','<?php echo CONFIG_SITEFILEURL;?>{$vo.filepath}')">预览</a> | <a class="J_ajax_del" href="{:U('Attachment/Atadmin/delete',array('aid'=>$vo['aid']))}">删除</a></td>
        </tr>
      </volist>
      </tbody>
    </table>
