...
class RbacAction extends AdminbaseAction {
        ...
        if (IS_POST) {
            ...
            if (is_array($_POST['menuid']) && count($_POST['menuid'])>0) {
                ...
                foreach ($_POST['menuid'] as $menuid) {
                    ...
                    //*****************************
                    $afile ='amenuid_'.$menuid;
                    $dfile ='dmenuid_'.$menuid;
                    $efile ='emenuid_'.$menuid;
                    $cfile ='cmenuid_'.$menuid;
                    $hfile ='hmenuid_'.$menuid;
                    $zdfile ='zdmenuid_'.$menuid;
                    $zfile ='zmenuid_'.$menuid;
                    $ofile ='omenuid_'.$menuid;
                    //******************************



                    $info['c_m'] = isset($_POST[$cfile]) ? $_POST[$cfile] : 1;
                    $info['a_m'] = (isset($_POST[$afile]) && $_POST[$afile] == $menuid) ?  'Y' : 'N';
                    $info['d_m'] = (isset($_POST[$dfile]) && $_POST[$dfile] == $menuid) ?  'Y' : 'N';
                    $info['e_m'] = (isset($_POST[$efile]) && $_POST[$efile] == $menuid) ?  'Y' : 'N';
                    $info['h_m'] = (isset($_POST[$hfile]) && $_POST[$hfile] == $menuid) ?  'Y' : 'N';
                    $info['zd_m'] = (isset($_POST[$zdfile]) && $_POST[$zdfile] == $menuid) ?  'Y ': 'N';
                    $info['z_m'] = (isset($_POST[$zfile]) && $_POST[$zfile] == $menuid) ?  'Y' : 'N';
                    $info['o_m'] = (isset($_POST[$ofile]) && $_POST[$ofile] == $menuid) ?  'Y' : 'N';
                    //print_r($info);
                    //die;
            ...
            foreach ($result as $n => $t) {
                ...
                $accessm = $this->get_access_m($t['id'],$roleid);
                $result[$n]['achecked'] = (isset($accessm['a_m']) && $accessm['a_m'] == 'Y') ? ' checked' : '';
                $result[$n]['dchecked'] = (isset($accessm['d_m']) && $accessm['d_m'] == 'Y') ? ' checked' : '';
                $result[$n]['echecked'] = (isset($accessm['e_m']) && $accessm['e_m'] == 'Y') ? ' checked' : '';
                $result[$n]['hchecked'] = (isset($accessm['h_m']) && $accessm['h_m'] == 'Y') ? ' checked' : '';
                $result[$n]['zdchecked'] = (isset($accessm['zd_m']) && $accessm['zd_m'] == 'Y') ? ' checked' : '';
                $result[$n]['zchecked'] = (isset($accessm['z_m']) && $accessm['z_m'] == 'Y') ? ' checked' : '';
                $result[$n]['ochecked'] = (isset($accessm['o_m']) && $accessm['o_m'] == 'Y') ? ' checked' : '';

                $result[$n]['c1'] = (isset($accessm['c_m']) && $accessm['c_m'] == '1') ? ' checked' : '';
                $result[$n]['c2'] = (isset($accessm['c_m']) && $accessm['c_m'] == '2') ? ' checked' : '';
                $result[$n]['c3'] = (isset($accessm['c_m']) && $accessm['c_m'] == '3') ? ' checked' : '';
                ...
                $str1 = "<tr id='node-\$id' \$parentid_node>
                           <td style='padding-left:30px;' width='40%'>\$spacer<input type='checkbox' name='menuid[]' value='\$id' level='\$level' \$checked onclick='javascript:checknode(this);'> \$name</td>
                           <td width='5%' >&nbsp;</td>
                           <td width='5%' >&nbsp;</td>
                           <td width='5%' >&nbsp;</td>
                           <td width='20%' >&nbsp;</td>
                ...
                $str = "<tr id='node-\$id' \$parentid_node>
                           <td style='padding-left:30px;' width='40%'>\$spacer<input type='checkbox' name='menuid[]' value='\$id' level='\$level' \$checked onclick='javascript:checknode(this);'> \$name</td>
                           <td width='5%' ><input type='checkbox' name='amenuid_\$id' value='\$id'  \$achecked >增加</td>
                           <td width='5%' ><input type='checkbox' name='dmenuid_\$id' value='\$id'  \$dchecked>删除</td>
                           <td width='5%' ><input type='checkbox' name='emenuid_\$id' value='\$id'  \$echecked>修改</td>
                           <td width='20%' ><input type='radio' name='cmenuid_\$id' value='1' \$c1>个人
                                              <input type='radio' name='cmenuid_\$id' value='2' \$c2>专线
                                              <input type='radio' name='cmenuid_\$id' value='3' \$c3>所有</td>
                           <td width='5%' ><input type='checkbox' name='hmenuid_\$id' value='\$id' \$hchecked>审核</td>
                           <td width='10%' ><input type='checkbox' name='zdmenuid_\$id' value='\$id' \$zdchecked>总删除</td>
                           <td width='5%' ><input type='checkbox' name='zmenuid_\$id' value='\$id' \$zchecked>支付</td>
                           <td width='5%' ><input type='checkbox' name='omenuid_\$id' value='\$id' \$ochecked>导出</td>
	    </tr>";



            $menu->init($result);
            $categorys = $menu->get_tree(0, $str,0,'','',$str1,1);
    ...
    protected function is_checked($data, $roleid, $priv_data) {
        ...
        foreach ($priv_data as $t) {
            $p[] = $t['mid'];
        }

        $info = in_array($data['id'], $p);
        if ($info) {
            return true;
        } else {
            return false;
     ...
     * @param int $menuid 菜单ID
     */
    protected  function is_parent($menuid){
        $data = $this->Menu->where(array("parentid" => $menuid))->select();
     ...
     * @param $roleid 权限编号
     */
    protected  function get_access_m($menuid,$roleid){
        if(empty($menuid) || empty($roleid)){
            return array();
        }

        $data = $this->Access->where(array("role_id" => $roleid,"mid"=>$menuid))->find(); //获取权限表数据
