...
class Tree {
     ...
     * @return string
     */
    public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '',$str1='',$sqx='') {
        $number = 1;
        //一级栏目
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
				...
				$child1 = $this->get_child($id);
				
				if(!empty($sqx)){
				    if(!is_array($child1)){
					    $parentid == 0 && $str_group ? eval("\$nstr = \"$str_group\";") : eval("\$nstr = \"$str\";");
					}else{
				        $parentid == 0 && $str_group ? eval("\$nstr = \"$str_group\";") : eval("\$nstr = \"$str1\";");
					}
                ...
                $this->ret .= $nstr;
                $nbsp = $this->nbsp;
                $this->get_tree($id, $str, $sid, $adds . $k . $nbsp, $str_group,$str1,$sqx);
                $number++;
            }
        }
        return $this->ret;
